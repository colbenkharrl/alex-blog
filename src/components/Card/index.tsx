...
Card.Link = function CardLink({ children, ...props }: CardLinkProps) {
  return (
    <>
      <div className="absolute -inset-x-4 -inset-y-6 z-0 scale-95 bg-amber-50 opacity-0 transition group-hover:scale-100 group-hover:opacity-80 dark:bg-lime-950/50 sm:-inset-x-6 sm:rounded-2xl" />
      <Link {...props}>
...
Card.CTA = function CardCTA({ children }: CardCTAProps) {
  return (
    <div
      aria-hidden="true"
      className="relative z-10 mt-4 flex items-center text-sm font-medium text-lime-700 dark:text-amber-400"
    >
      {children}
...
Card.Eyebrow = function CardEyebrow<T extends React.ElementType = 'p'>({
  ...
  return (
    <Component
      className={clsx(
        className,
        'relative z-10 order-first mb-3 flex items-center text-sm text-lime-800 dark:text-amber-400/80',
        decorate && 'pl-3.5',
      )}
      {...props}
    >
